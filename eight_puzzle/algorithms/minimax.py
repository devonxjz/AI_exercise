...
class MinimaxSolver(BaseSolver):
    """
    Adapter implementing Minimax search for the 8-puzzle.
    ...
    def step_search(self):
        ...
        if curr_grid == self.goal_state:
            ...
            log_entry["log"] = f"🎯 Success! Goal state reached via Minimax.\nPath length: {len(self.solution_path) - 1} moves.\nSteps taken: {self.steps_taken}\nNodes expanded: {self.nodes_expanded}"
            return log_entry

        if self.steps_taken >= self.max_steps:
            self.state = "unsolvable"
            log_entry["status"] = "failed"
            log_entry["log"] = f"❌ Minimax failed to reach goal within {self.max_steps} steps."
            return log_entry

        # Generate neighbor states (moves) and run minimax
        ...
        for next_grid, action in neighbors:
            val = self.minv(next_grid, depth=1)
        ...
        # Sort frontier_sample descending by minimax value
        ...
        if best_state is None:
            self.state = "unsolvable"
            log_entry["status"] = "failed"
            log_entry["log"] = "❌ Minimax search returned no valid moves."
            return log_entry
        ...
        log_entry["log"] = f"🤖 Minimax at step {self.steps_taken} chose move {best_action} with Minimax value {best_value}."
        return log_entry
    ...
    def maxv(self, state, depth):
        ...
        for next_grid, _ in neighbors:
            val = max(val, self.minv(next_grid, depth + 1))
        return val
    ...
    def minv(self, state, depth):
        ...
        val = float('inf')
        for next_grid, _ in neighbors:
            val = min(val, self.maxv(next_grid, depth + 1))
        return val
