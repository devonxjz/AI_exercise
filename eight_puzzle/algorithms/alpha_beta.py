...
class AlphaBetaSolver(BaseSolver):
    """
    Adapter implementing Alpha-Beta pruning search for the 8-puzzle.
    At each step, MAX player looks ahead to choose a move that maximizes the utility
    under the assumption that a hypothetical MIN player chooses moves that minimize the utility,
    using alpha-beta pruning to eliminate subtrees that cannot influence the final decision.
    """
    ...
    def step_search(self):
        ...
        if curr_grid == self.goal_state:
            ...
            log_entry["log"] = f"🎯 Success! Goal state reached via Alpha-Beta.\nPath length: {len(self.solution_path) - 1} moves.\nSteps taken: {self.steps_taken}\nNodes expanded: {self.nodes_expanded}"
            return log_entry

        if self.steps_taken >= self.max_steps:
            self.state = "unsolvable"
            log_entry["status"] = "failed"
            log_entry["log"] = f"❌ Alpha-Beta failed to reach goal within {self.max_steps} steps."
            return log_entry

        # Generate neighbor states (moves) and run alpha-beta search
        ...
        for next_grid, action in neighbors:
            val = self.minv(next_grid, depth=1, alpha=alpha, beta=beta)
        ...
        if best_state is None:
            self.state = "unsolvable"
            log_entry["status"] = "failed"
            log_entry["log"] = "❌ Alpha-Beta search returned no valid moves."
            return log_entry
        ...
        log_entry["log"] = f"🤖 Alpha-Beta at step {self.steps_taken} chose move {best_action} with value {best_value}."
        return log_entry
    ...
    def maxv(self, state, depth, alpha, beta):
        ...
        for next_grid, _ in neighbors:
            val = max(val, self.minv(next_grid, depth + 1, alpha, beta))
            if val >= beta:
                return val
            alpha = max(alpha, val)
        return val

    def minv(self, state, depth, alpha, beta):
        self.nodes_expanded += 1
        if state == self.goal_state:
            return 10000 - depth
        if depth >= self.search_depth:
        ...
        val = float('inf')
        for next_grid, _ in neighbors:
            val = min(val, self.maxv(next_grid, depth + 1, alpha, beta))
            if val <= alpha:
                return val
            beta = min(beta, val)
        return val
